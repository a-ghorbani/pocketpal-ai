...
describe('compareReports', () => {
  it('should pass when within budget and no leak', () => {
    ...
    const current = makeReport({
      checkpoints: [
        ...makeReport().checkpoints.slice(0, 4),
        {
          ...
          timestamp: new Date().toISOString(),
          native: {
            phys_footprint: 1200 * 1024 * 1024, // 1200 MB > 1024 MB default
  ...
  it('should detect memory leak when post-unload > 110% pre-load', () => {
    ...
    // Set model_unloaded to 120% of app_launch
    checkpoints[6] = {
      label: 'model_unloaded',
      timestamp: new Date().toISOString(),
      native: {
        phys_footprint: 125 * 1024 * 1024, // 125 MB = 125% of 100 MB
  ...
  it('should pass when post-unload is exactly at 110% threshold', () => {
    const checkpoints = makeReport().checkpoints.map(c => ({...c}));
    // Set model_unloaded to exactly 110% of app_launch
    checkpoints[6] = {
      label: 'model_unloaded',
      timestamp: new Date().toISOString(),
      native: {
        phys_footprint: 110 * 1024 * 1024, // 110 MB = exactly 110% of 100 MB
        available_memory: 3e9,
      },
    };
  ...
  it('should handle missing app_launch or model_unloaded checkpoints', () => {
    ...
    // Should not crash, leak_detected should be false
    expect(result.leak_detected).toBe(false);

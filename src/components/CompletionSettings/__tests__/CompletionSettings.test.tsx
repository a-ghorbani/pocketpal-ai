...
describe('CompletionSettings', () => {
  ...
  describe('server defaults', () => {
    ...
    it('does not call a quantised float a deliberate change', () => {
      const {getByTestId} = render(
        <CompletionSettings
          settings={{...mockCompletionParams, penalty_repeat: 1.1}}
          onChange={jest.fn()}
          serverDefaults={{penalty_repeat: 1.104}}
        />,
      );

      expect(getByTestId('penalty_repeat-server-default')).toHaveTextContent(
        'server default',
      );
    });
    ...
    it("shows the server's out-of-range default when the value is left to it", () => {
      const onChange = jest.fn();
      ...
      // `--top-k 0` is how llama.cpp disables top-k. The app's range starts at
      // 1, but top_k is not sent, so 0 is what runs.
      expect(getByTestId('top_k-server-default')).toHaveTextContent(
        'Using server default: 0',
      );
      expect(queryByTestId('top_k-server-default-reset')).toBeNull();
    ...
    it('resets to the reported value at the precision its control holds', () => {
      const onChange = jest.fn();
      const wire = propsModelDescribing.default_generation_settings.params;
      const {getByTestId} = render(
        <CompletionSettings
      ...
      expect(onChange).toHaveBeenCalledWith('temperature', 0.8);
    });

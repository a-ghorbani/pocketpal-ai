...
const createFloatingStyles = (theme: Theme, selected?: boolean) =>
  StyleSheet.create({
    ...
    // Positioned via the animated physical `left` (mirrored in RTL) at runtime;
    // see BottomNavBar.pillLeft. top/bottom equal the root padding so the pill
    // spans exactly the item row height.
    pill: {
      position: 'absolute',

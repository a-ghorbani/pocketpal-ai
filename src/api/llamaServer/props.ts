 ...
 */
function readSamplerDefaults(generationSettings: any): Samplers {
  const defaults: Samplers = {};
 ...
 * Never throws: a timeout, a non-2xx (a 401 included) or a malformed body all
 * resolve to no fields at all, so a failure is never evidence of absence and a
 * probe can never blank a capability the caller already knows. `/props` is
 * llama.cpp-specific; a caller asks `profileFor(type).hasProps` before
 * invoking, never the type itself.
 *

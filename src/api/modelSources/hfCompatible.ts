...
import {fetchWithTimeout, MODEL_SOURCE_REQUEST_TIMEOUT_MS} from './network';
...
export async function fetchHFCompatibleModels({
  ...
  const response = await axios.get(
    ...
    {
      // The cursor URL already encodes all search state; resending params
      // would duplicate/conflict with it.
      params: nextPageUrl
        ? undefined
        : {
            search,

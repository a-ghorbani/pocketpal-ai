...
describe('SettingsScreen', () => {
  ...
  it('caps slider max to n_ctx when n_ctx < 4096', async () => {
    jest.useFakeTimers();
    const {getByTestId, getByText} = render(<SettingsScreen />, {
      withSafeArea: true,
      withNavigation: true,
    ...
    // Set n_ctx to a value less than 4096
    ...
    // The slider should exist (we can't directly access its max prop in testing,
    // but we can verify the slider respects the implementation by checking
    // that the component renders with the correct context)
    const slider = getByTestId('image-max-tokens-slider');
    expect(slider).toBeTruthy();

    // Verify that attempting to set a value above n_ctx would be handled
    act(() => {
      fireEvent(slider, 'onValueChange', 2048);
    });
    ...
    expect(modelStore.setImageMaxTokens).toHaveBeenCalledWith(2048);
  });

  it('shows feedback message when value >= n_ctx', async () => {
    jest.useFakeTimers();
    ...
    // Set image_max_tokens to be >= n_ctx
    modelStore.contextInitParams.n_ctx = 2048;
    modelStore.contextInitParams.image_max_tokens = 2048;
    ...
    await waitFor(() => {
      expect(getByText(/Limited by context size/i)).toBeTruthy();
    });
  });
});

...
class HFStore {
      ...
      if (model.siblings.length === 0 && fileDetails.length > 0) {
        const updatedSiblings = await Promise.all(
          createSiblingsFromFileDetails(
            ...
            fileDetails,
            // Keep download URLs on the model's own source (without this,
            // hf-mirror files fall back to huggingface.co URLs).
            getModelSourceConfig(source).domain,
          ).map(async file => ({
            ...file,
  ...
  // NOTE: the stored token is a Hugging Face token for huggingface.co.
  // ModelScope uses a different account system and hf-mirror.com is a
  // third-party mirror, so the token is only ever sent for the huggingface
  // source. (DownloadManager enforces the same rule by download URL as a
  // second line of defense.)
  shouldUseTokenForSource(source: ModelSourceId): boolean {
    return source === 'huggingface' && this.shouldUseToken;
  }
  ...
  // Fetch the models from the selected source
  async fetchModels() {

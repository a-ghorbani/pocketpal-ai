 ...
 *
 * Reads `model.reasoning`. When absent the legacy `supportsThinking` boolean
 * is the fail-open fallback: `true` → 'yes', `false` → 'no', absent →
 * 'unknown'. Axis-2 (effort) is reported only when axis-1 is not 'no'.
 */

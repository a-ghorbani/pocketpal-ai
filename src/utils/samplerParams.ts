...
export function isLeftToServer(param: SamplerParam, value: number): boolean {
  const leftToServer = valueLeftToServer(param);
  return leftToServer !== undefined && value === leftToServer;
}
...
export function pickSamplers(params: CompletionParams): Samplers {
  ...
  for (const param of SAMPLER_PARAMS) {
    ...
    if (value !== undefined && !isLeftToServer(param, value)) {
